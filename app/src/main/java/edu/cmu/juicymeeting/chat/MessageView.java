...
public class MessageView {
    ...
    private Message.RecipientStatus getMessageStatus(Message msg) {
        ...
        if (!msg.getSender().getUserId().equalsIgnoreCase(GroupChatActivity.getUserID()))
        ...
        for (int i = 0; i < GroupChatActivity.getAllParticipants().size(); i++) {

            //Don't check the status of the current user
            String participant = GroupChatActivity.getAllParticipants().get(i);
            if (participant.equalsIgnoreCase(GroupChatActivity.getUserID()))
                continue;
